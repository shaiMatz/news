...
class DatabaseStorage {
  ...
  async getNewsByLocation(latitude, longitude, radiusKm = 50, maxRadius = 500) {
    // Retrieve all news items that have coordinates
    ...
    // Filter the news items by distance from the provided coordinates
    const filteredNews = news.filter(item => {
      ...
      // Calculate the approximate distance (km) using a simple Euclidean formula
      const distance = Math.sqrt(
        Math.pow(newsJson.coordinates.latitude - latitude, 2) +
        Math.pow(newsJson.coordinates.longitude - longitude, 2)
      ) * 111; // Rough conversion factor from degrees to kilometers

      return distance <= radiusKm;
    });

    if (filteredNews.length === 0 && radiusKm < maxRadius) {
      return await this.getNewsByLocation(latitude, longitude, radiusKm * 2, maxRadius);
    }

    return filteredNews.map(item => item.toJSON());
  }
